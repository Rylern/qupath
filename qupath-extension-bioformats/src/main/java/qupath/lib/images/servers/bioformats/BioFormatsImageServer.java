...
public class BioFormatsImageServer extends AbstractTileableImageServer {
	 ...
	 */
	private final BioFormatReaderPool readerPool;
	...
	private BioFormatsImageServer(URI uri, final BioFormatsServerOptions options, String... args) throws IOException, URISyntaxException {
		...
		readerPool = new BioFormatReaderPool(options, filePathOrUrl, bfArgs);
		var readerWrapper = readerPool.getDefaultReaderWrapper();
		if (readerWrapper.isEmpty()) {
			throw new RuntimeException("Could not retrieve main reader");
		}
		IFormatReader reader = readerWrapper.get().getReader();
...
//					args(args).
	...
	public BufferedImage readTile(TileRequest tileRequest) {
		return readerPool.openImage(tileRequest, series, nChannels(), isRGB(), colorModel).orElse(null);
	}
	...
	public BufferedImage getAssociatedImage(String name) {
		...
		return readerPool.openSeries(associatedImageMap.get(name)).orElse(null);
	}
